...
st.markdown(
    ...
    <div class="hero-box" style="display: flex; align-items: center; gap: 20px;">
        ...
        <div>
            ...
            <h3 style="margin:0; color:#C00E19; font-size: 1.2rem;">Facultad de Ingeniería y Ciencias Aplicadas</h3>
...
overview_tab, theory_tab, digital_tab, survey_tab, compare_tab, models_tab, explorer_tab = st.tabs(
    [
        "Resumen ejecutivo",
        "Modelo teórico",
        "Panorama digital",
...
with theory_tab:
    st.subheader("Fundamentación del Modelo Teórico")
    st.markdown(
        """
        <div class="section-note">
            <em>"Estos constructos no solo orientan la interpretación teórica del fenómeno, sino que también sirven de base para la operacionalización de variables y para la aplicación posterior de modelos de análisis estadístico y aprendizaje automático."</em>
        </div>
        """,
        unsafe_allow_html=True,
    )
    
    st.markdown("El siguiente diagrama expone la relación teórica entre los constructos de campaña digital y su cristalización en la confianza electoral de la ciudadanía:")
    
    st.graphviz_chart('''
    digraph G {
        rankdir="LR";
        node [shape=box, style="filled,rounded", color="#0076BD", fillcolor="#F2F7FA", fontcolor="#12344D", fontname="Helvetica"];
        edge [color="#3c556b", fontname="Helvetica", fontsize=10];
        
        subgraph cluster_digital {
            label = "Entorno de Campañas Digitales\n(Scraping Social)";
            style="dashed";
            color="#C00E19";
            fontcolor="#C00E19";
            
            A [label="Uso de IA y\nAutomatización"];
            B [label="Desinformación y\nDeepfakes"];
            C [label="Sentimiento del\nDebate Digital"];
        }
        
        subgraph cluster_social {
            label = "Percepción Ciudadana\n(Encuestas)";
            style="dashed";
            color="#C00E19";
            fontcolor="#C00E19";
            
            D [label="Percepción de\nManipulación"];
            E [label="Demanda de\nRegulación"];
        }
        
        F [label="Nivel de Confianza\nElectoral local", style="filled,rounded,bold", color="#C00E19", fillcolor="#FFFFFF"];
        
        A -> D [label="  influye en"];
        B -> D [label="  condiciona"];
        C -> F [label="  contextualiza"];
    ...
    st.markdown("---")
    st.markdown("**Operacionalización para Machine Learning:**")
    st.markdown("- **Variables Predictoras (Features):** Métricas de percepción sobre manipulación técnica, conocimiento sobre IA, uso de plataformas y variables sociodemográficas.")
    st.markdown("- **Variable Objetivo (Target):** Nivel de Confianza Electoral (Baja, Media, Alta).")
